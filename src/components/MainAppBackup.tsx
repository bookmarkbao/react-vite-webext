...
import { Switch, Collapse, Button, Input, Select, Tooltip, Badge } from "antd";
...
export const MainApp = () => {
  ...
  // 对象序列号后保存
  // setAjaxInterceptorRules(JSON.parse(JSON.stringify(settings.ajaxInterceptor_rules)));
  // console.log(`setAjaxInterceptorRules`, settings.ajaxInterceptor_rules)
  // settings.init &&
  ...
  const renderPanelHeader = (item, index) => {
    ...
    return (
      ...
      >
        ...
        {/* {settings.ajaxInterceptor_rules.interceptedRequests[match] && renderInterceptorInfo(settings.ajaxInterceptor_rules, match)} */}
      </Panel>
    );
  };
  ...
  const renderContent = () => {
    ...
    return (
      <Collapse defaultActiveKey={["1"]} onChange={() => ({})}>
        {ajaxInterceptorRules.map((item, index) =>
