...
export class XHRProxyHandler<T extends XMLHttpRequest> implements ProxyHandler<T> {
  public XMLReq: XMLHttpRequest; // 保存原对象的实例化
  ...
  public get(target: T, key: string) {
    ...
    switch (key) {
      ...
      default:
        ...
        if ((key === "response" || key === "responseText") && this.item._switchOn) {
          return this.item._response;
        }
  ...
  protected triggerUpdate() {
    if (!this.item.noVConsole) {
      this.onUpdateCallback(this.item, ({ _switchOn, _response }: { _switchOn: boolean; _response: string }) => {
        this.item._switchOn = _switchOn;
        this.item._response = _response;

        console.log(`回调了=====`, this.item);
      });
    }
  }
...
export class XHRProxy {
  ...
  public static create(onUpdateCallback: IOnUpdateCallback) {
    console.log("原生的===", XMLHttpRequest); // 原生的
    // 发一个请求，实力化一次
    return new Proxy(XMLHttpRequest, {
      construct(ctor) {
        // ctor 就是XMLHttpRequest
        // new之后，得到一个xhr对象。 这个对象，也被代理啦
        // ctor ƒ XMLHttpRequest() { [native code] }
        // console.log(ctor);
        const XMLReq = new ctor();
        // return new target(...args);
        // 第二个代理：用于拦截XMLReq对象 属性和方法。操作XMLReq，交给XHRProxyHandler去处理
        // XHRProxyHandler传入XMLReq对象，传入回到函数onUpdateCallback
        // return XMLReq
